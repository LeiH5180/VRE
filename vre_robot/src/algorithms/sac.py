...
class SAC(object):
	...
	def update_encoder(self, obs_ori, action_ori, obs_2, action_2, L=None, step=None):
		obs_2 = torch.cat((obs_ori, obs_2), dim=0)
		action_2 = torch.cat((action_ori, action_2), dim=0)
		obs_2_aug = augmentations.random_shift(obs_2.clone())
		for param in self.critic.parameters():    # 冻结critic网络参数的更新
			param.requires_grad_(False)
		for param in self.critic.encoder.parameters():    # 只更新critic网络encoder的参数
			param.requires_grad_(True)
		current_Q1_ori, current_Q2_ori = self.critic(obs_2, action_2)
		current_Q1_ori_aug, current_Q2_ori_aug = self.critic(obs_2_aug, action_2)
		loss_critic_encoder = (F.mse_loss(current_Q1_ori.detach(), current_Q1_ori_aug) + F.mse_loss(current_Q2_ori.detach(), current_Q2_ori_aug)) * 0.1
	...
	def update(self, replay_buffer, L, step):
		...
		if step % self.encoder_update_freq == 0 and self.add_VRE:
			...
			self.update_encoder(obs, action, obs_2, action_2, L=None, step=None)    # 单独更新编码器		
