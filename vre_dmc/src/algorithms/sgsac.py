...
class SGSAC(SAC):
    def __init__(self, obs_shape, action_shape, args):
        ...
        self.attribution_predictor = m.AttributionPredictor(action_shape[0],self.critic.encoder).cuda(3)
    ...
    def update_aux(self, obs, action, obs_grad, mask, step=None, L=None):
        ...
        s_tilde_1 = augmentations.random_overlay(obs.clone())
        # s_tilde_2 = augmentations.random_conv(obs.clone())    # ----------------------------
        self.aux_optimizer.zero_grad()
        pred_attrib_1, aux_loss_1 = self.compute_attribution_loss(s_tilde_1,action, mask)
        # pred_attrib_2, aux_loss_2 = self.compute_attribution_loss(s_tilde_2,action, mask)    # ----------------------------
        # aux_loss = aux_loss_1*0.5 + aux_loss_2*0.5    # ----------------------------
        aux_loss = aux_loss_1    # ----------------------------
        aux_loss.backward()
        ...
        #     self.log_tensorboard(s_tilde_1, action, step, prefix="augmented")    # ------------------------------
        #     self.log_tensorboard(s_tilde_2, action, step, prefix="augmented")    # ------------------------------
