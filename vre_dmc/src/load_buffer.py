...
def main():
    ...
    rb_tensor = torch.cat([torch.tensor([(np.array(obses_rb[i][0]).reshape(3,3,84,84)) for i in range(batch_size)]).reshape(-1,3,84,84)
                          ,torch.tensor([(np.array(obses_rb[i][1]).reshape(3,3,84,84)) for i in range(batch_size)]).reshape(-1,3,84,84)],dim=0).float().to('cuda:1')
    tb1_tensor = torch.cat([torch.tensor([(np.array(obses_tb1[i][0]).reshape(3,3,84,84)) for i in range(batch_size)]).reshape(-1,3,84,84)
                          ,torch.tensor([(np.array(obses_tb1[i][1]).reshape(3,3,84,84)) for i in range(batch_size)]).reshape(-1,3,84,84)],dim=0).float().to('cuda:1')
    tb2_tensor = torch.cat([torch.tensor([(np.array(obses_tb2[i][0]).reshape(3,3,84,84)) for i in range(batch_size)]).reshape(-1,3,84,84)
                          ,torch.tensor([(np.array(obses_tb2[i][1]).reshape(3,3,84,84)) for i in range(batch_size)]).reshape(-1,3,84,84)],dim=0).float().to('cuda:1')
    tb3_tensor = torch.cat([torch.tensor([(np.array(obses_tb3[i][0]).reshape(3,3,84,84)) for i in range(batch_size)]).reshape(-1,3,84,84)
                          ,torch.tensor([(np.array(obses_tb3[i][1]).reshape(3,3,84,84)) for i in range(batch_size)]).reshape(-1,3,84,84)],dim=0).float().to('cuda:1')
    ...
    all_out = torch.cat([out_rb,out_tb1,out_tb2,out_tb3],dim=0).cpu()
    ...
    df ["label"] = None
    df ["label"][:length] = "train_state"
    df ["label"][length:2*length] = "color_hard"
    df ["label"][2*length:3*length] = "video_easy"
    df ["label"][3*length:] = "video_hard"

    palette = sns.color_palette("Set1",5)
    sns.set_palette(palette)
    label_mode = ["color_hard","video_easy","video_hard"]
    palette=['#DC565F', '#3E61AC']

    for i in range(3):       
        df_d = pd.concat([df[df["label"] == "train_state"],df[df["label"] == label_mode[i]]], ignore_index=True)
        ...
        plt.savefig(f"train--{label_mode[i]}.png", dpi=300)
        plt.close()
